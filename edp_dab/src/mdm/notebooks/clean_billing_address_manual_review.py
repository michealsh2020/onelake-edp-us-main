...
# MAGIC %md
# MAGIC ## Billing Review Function

# COMMAND ----------

def pull_billing_records_for_review():
    query = f"""select sid_id, billing_street, billing_street_clean, review_billing_street_clean_flag,
    case when billing_street IS NOT NULL AND billing_street_clean is null then 'Billing Street Null'
         when billing_street_clean IS NOT NULL AND billing_street_clean NOT LIKE '%0%'
                                               AND billing_street_clean NOT LIKE '%1%'
                                               AND billing_street_clean NOT LIKE '%2%'
                                               AND billing_street_clean NOT LIKE '%3%'
                                               AND billing_street_clean NOT LIKE '%4%'
                                               AND billing_street_clean NOT LIKE '%5%'
                                               AND billing_street_clean NOT LIKE '%6%'
                                               AND billing_street_clean NOT LIKE '%7%'
                                               AND billing_street_clean NOT LIKE '%8%'
                                               AND billing_street_clean NOT LIKE '%9%' then 'does not Contains Number'
     end as billing_street_rule_broken,
    
    billing_city, billing_city_clean, 
    case when billing_city IS NOT NULL AND billing_city_clean is null then 'Billing City Null'
         when billing_city_clean_number_flag = true then 'Contains Number'
    end as billing_city_rule_broken,
    
    billing_state, billing_state_clean, review_billing_state_clean_flag,
    case when billing_state IS NOT NULL AND billing_state_clean is null then 'Billing State Null'
        when billing_state_clean_number_flag = true then 'Contains Number'
        when invalid_ca_billing_state_flag = true then 'Invalid CA State'
        when invalid_au_billing_state_flag = true then 'Invalid AU State'
        when invalid_us_billing_state_flag = true then 'Invalid US State'
    end as billing_state_rule_broken,
    
    billing_country, billing_country_clean, 
    case when billing_country IS NOT NULL AND billing_country_clean is null then 'Billing Country Null'
    when billing_country_clean_number_flag = true then 'Contains Number'
    end as billing_country_rule_broken,
    
    billing_postal_code, billing_postal_code_clean,review_invalid_billing_postal_code_clean_flag,
    case when billing_postal_code IS NOT NULL AND billing_postal_code_clean is null then 'Billing PostalCode Null'
    end as billing_postal_code_rule_broken   
  
    from {source_catalog}.tmp.sf_account_clean_address_for_review 
    where (review_billing_state_clean_flag = true
    or review_invalid_billing_postal_code_clean_flag = true 
    or billing_country_clean_number_flag = true
    or billing_city_clean_number_flag = true
    or review_billing_street_clean_flag = true
    or(billing_street_clean NOT LIKE '%0%'
        AND billing_street_clean NOT LIKE '%1%'
        AND billing_street_clean NOT LIKE '%2%'
        AND billing_street_clean NOT LIKE '%3%'
        AND billing_street_clean NOT LIKE '%4%'
        ...
        AND billing_street_clean NOT LIKE '%7%'
        AND billing_street_clean NOT LIKE '%8%'
        AND billing_street_clean NOT LIKE '%9%')
    )and review_status is null
     
    """
    df_records_to_review = spark.sql(query)
    #return df_records_to_review.toPandas()
...
result = pull_billing_records_for_review() # Ensure SQL query is complete 
display(result)
...
# MAGIC %md
# MAGIC # Manual Review Process - Billing
...
# GET DATA TO REVIEW 
review_data_pd = pull_billing_records_for_review().toPandas() 
...
orig_cols = ['sid_id', 'billing_street', 'billing_city', 'billing_state', 'billing_country', 'billing_postal_code']
clean_cols = ['sid_id', 'billing_street_clean', 'billing_city_clean', 'billing_state_clean', 'billing_country_clean', 'billing_postal_code_clean']
rule_cols = ['sid_id', 'billing_street_rule_broken', 'billing_city_rule_broken', 'billing_state_rule_broken', 'billing_country_rule_broken', 'billing_postal_code_rule_broken']
...
schema = StructType([
    ...
    StructField("review_billing_street_clean_flag", BooleanType(), True), 
    StructField("billing_street_rule_broken", StringType(), True), 
    StructField("billing_city", StringType(), True), 
    StructField("billing_city_clean", StringType(), True), 
    StructField("billing_city_rule_broken", StringType(), True), 
    StructField("billing_state", StringType(), True), 
    StructField("billing_state_clean", StringType(), True), 
    StructField("review_billing_state_clean_flag", BooleanType(), True), 
    StructField("billing_state_rule_broken", StringType(), True), 
    StructField("billing_country", StringType(), True), 
    StructField("billing_country_clean", StringType(), True), 
    StructField("billing_country_rule_broken", StringType(), True), 
    StructField("billing_postal_code", StringType(), True), 
    StructField("billing_postal_code_clean", StringType(), True), 
    StructField("review_invalid_billing_postal_code_clean_flag", BooleanType(), True), 
    StructField("billing_postal_code_rule_broken", StringType(), True), 
    StructField("billing_street_selected", StringType(), True), 
    StructField("billing_city_selected", StringType(), True), 
    StructField("billing_state_selected", StringType(), True), 
    StructField("billing_country_selected", StringType(), True),
    StructField("billing_postal_code_selected", StringType(), True)
])
...
updated_review_data_spark_df.display()
...
try:
    ...
    update_columns = {
        "billing_street_clean": "source.billing_street_selected",
        "billing_city_clean": "source.billing_city_selected",
        "billing_state_clean": "source.billing_state_selected",
        "billing_postal_code_clean": "source.billing_postal_code_selected",
        "billing_country_clean": "source.billing_country_selected",
        "address_last_cleaned_timestamp" : current_timestamp(),
        "review_status" : lit("Reviewed"),
        "reviewed_by" : lit(reviewed_by_cleaned),
        "final_clean_flag" : lit(0),
        "needs_manual_review" : lit("false"),
    ...
    print(f"Successfully merged updates into table: {source_catalog}.tmp.sf_account_clean_address_for_review")
...
# MAGIC %md
# MAGIC ## Validation - View the Reviewed Record

# COMMAND ----------

'''%sql
select sid_id, billing_street_clean, billing_city_clean, billing_state_clean, billing_country_clean, billing_postal_code_clean
from f"{source_catalog}.tmp.sf_account_clean_address_for_review
where sid_id in ('0014Q00002IkpcrQAB','0014Q00002IkpmhQAB','00130000012ybfiAAA','0014Q00002Y9ep5QAB','00130000012yZhcAAE')
'''
